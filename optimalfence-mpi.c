...
bool CalcularCombinacionOptima(int PrimeraCombinacion, int UltimaCombinacion, PtrListaArboles Optimo)
{
	...
	int numtasks, namelen, rank, i, tag = 111;
	char hostname[256];
	char buffer[100];
	int position;
	int start_position;
	...
	int blocksize = UltimaCombinacion/(numtasks-1);
	if (rank == 0)
	{
		for (int i = 1; i < numtasks; ++i)
		{
			start_position = PrimeraCombinacion+(i-1)*(blocksize+1);
			MPI_Send(&start_position, 1, MPI_INT, i, tag, MPI_COMM_WORLD);
		}

		CosteMejorCombinacion = Optimo->Coste;
		for (i = 1; i < numtasks; ++i)
		{
			MPI_Recv((void*)buffer, 100, MPI_PACKED, MPI_ANY_SOURCE, tag, MPI_COMM_WORLD, &status);
			position = 0;
			MPI_Unpack(buffer, 100, &position, &pos, 1, MPI_INT, MPI_COMM_WORLD);
			MPI_Unpack(buffer, 100, &position, &local_cmc, 1, MPI_INT, MPI_COMM_WORLD);
			if (local_cmc < CosteMejorCombinacion)
			{
				CosteMejorCombinacion = local_cmc;
				MejorCombinacion = pos;
			}
		}
	...
	{
		MPI_Recv(&start_position, 1, MPI_INT, 0, tag, MPI_COMM_WORLD, &status);

		local_cmc = Optimo->Coste;
		for (i = 0; i < blocksize+1 && i < UltimaCombinacion; ++i)
		{
			Coste = EvaluarCombinacionListaArboles(start_position+i);
			if (Coste < local_cmc) 
			{
				local_cmc = Coste;
		...
		MPI_Pack(&pos, 1, MPI_INT, buffer, 100, &position, MPI_COMM_WORLD);
		MPI_Pack(&local_cmc, 1, MPI_INT, buffer, 100, &position, MPI_COMM_WORLD);
		MPI_Send((void*)buffer, 100, MPI_PACKED, 0, tag, MPI_COMM_WORLD);
	}
	MPI_Finalize();
...
void MostrarArboles(TListaArboles CombinacionArboles)
{
  ...
  for (;a<ArbolesEntrada.NumArboles;a++) {int x = 1;}
   // printf("  ");  
}
